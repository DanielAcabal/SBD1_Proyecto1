...
SELECT h.nombre,h.direccion,count(e.nombre) AS fallecidos FROM hospital h
JOIN victima v ON h.ID_HOSPITAL=v.ID_HOSPITAL 
JOIN estado e ON v.id_estado=e.id_estado
WHERE v.FECHA_MUERTE IS NOT NULL OR  e.nombre='Muerte' GROUP BY h.nombre,h.DIRECCION ;
...
JOIN ESTADO e ON v.id_estado=e.id_estado
WHERE v.FECHA_MUERTE IS NOT NULL OR  e.nombre='Muerte' HAVING count(r.ID_ASOCIADO)>3 GROUP BY v.nombre,v.apellido,v.direccion ;
...
JOIN VICTIMA v ON r.ID_VICTIMA=v.ID_VICTIMA 
JOIN ESTADO e ON v.id_estado=e.id_estado
WHERE e.nombre='Suspendida' AND c.NOMBRE='Beso' GROUP BY v.nombre,v.apellido,v.direccion;
...
JOIN VICTIMA v ON o.ID_VICTIMA=v.ID_VICTIMA 
GROUP BY v.NOMBRE,v.APELLIDO,v.FECHA_SOSPECHA) 
ON t.tratamientos=ma OR t.tratamientos=mi ORDER BY t.tratamientos desc;
